...
function canCast(SpellID,KnownSkip,MovementCheck)
	-- local lagTolerance = getValue("Lag Tolerance") or 0;
  	if (KnownSkip == true or isKnown(SpellID)) and IsUsableSpell(SpellID) and (MovementCheck == false or GlobalCooldown == 0 or isMoving("player") ~= true or UnitBuffID("player",79206) ~= nil) then
      	return true;
    end
end
...
function castSpell(Unit,SpellID,FacingCheck,MovementCheck,SpamAllowed,KnownSkip)
	if shouldStopCasting(SpellID) ~= true and not UnitIsDeadOrGhost(Unit) and GDUnitIsFollow()==true then
